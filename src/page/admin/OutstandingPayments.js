...
export default class OutstandingPayments extends Component {
  constructor(props) {
    super(props);
    this.state = {
      search: "",
  ...
  Get = async (page) => {
    ...
    let result = await GET("api/admin/outstanding-payments/list", body);
    if (result?.status) {
  ...
  render() {
    return (
      <div className="w-100 min-vh-100">
        <Navbar header="Transaction History" page1="หน้าหลัก" page2="รายการค้างชำระ" />
        <div className="body px-3 pb-3 pt-0">
          <h4 className="fw-bold mb-2">รายการค้างชำระ</h4>
          <div className="d-flex align-items-center flex-wrap">
            ...
            <div className="d-flex ms-auto">
              <button className="btn btn-primary px-4 me-2 mb-2" onClick={async () => {}}>
                Export Excel
              </button>
              <button className="btn btn-warning px-4 me-2 mb-2" onClick={async () => {}}>
                Export PDF
              </button>
            </div>
          </div>
                ...
                <tbody>
                  {this.state.data?.map((item, index) => (
                    <tr key={"table_" + index}>
                      ...
                      <td>{item.datetime_create ? format_date(item.datetime_create):""}</td>
